...
from ml.commons.utils.multi_gpu import load_parallel_model, set_model_state, get_current_state
...
class Loader:
    ...
    def default(self, model, optimizer):
        ...
        sys.stdout.write("DEFAULT State Loaded at epoch {}, Lr {}".format(starting_epoch, step))
        return model, optimizer, starting_epoch, step, learning_rate

    @LogDecorator()
    def resume(self, model, optimizer):
        saved_trainer_state = self.get_saved_state(self.config.chk_path)
        ...
        learning_rate = optimizer.defaults["lr"]
        sys.stdout.write("RESUME State Loaded at epoch {}, Lr {}".format(starting_epoch, step))
        return model, optimizer, starting_epoch, step, learning_rate

    @LogDecorator()
    def get_saved_state(self, weight_path):
        assert os.path.exists(weight_path), "NO SAVED STATE FOUND TO LOAD"
