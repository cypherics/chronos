...
https://github.com/asanakoy/kaggle_carvana_segmentation/blob/master/albu/src/data_transformation.py
"""
...
class RescaleCrop:
    ...
    def __call__(self, img, mask):
        ...
        if random.random() < self.prob:
            ...
            img = crop_image(img, (dim, dim), (height, width))
            mask = crop_image(mask, (dim, dim), (height, width))
            img, mask = perform_scale(img, mask, (dim_h, dim_w))
        return img, mask
...
class GaussianNoise:
    ...
    def __call__(self, image, mask):
        if random.random() < self.prob:
            image = iaa.AdditiveGaussianNoise(
...
class PerspectiveTransform:
    ...
    def __call__(self, image, mask):
        if random.random() < self.prob:
            image = iaa.PerspectiveTransform(
                scale=self.scale, keep_size=True
            ).augment_image(image)
            mask = iaa.PerspectiveTransform(
                scale=self.scale, keep_size=True
            ).augment_image(mask)
        return image, mask
...
def perform_scale(img, label, dimension, interpolation=cv2.INTER_LINEAR):
    ...
    lbl = cv2.resize(label, (new_width, new_height), interpolation=interpolation)
        ...
        [
            OneOrOther(
                *(
                    ...
                    ShiftScaleRotate(
                        shift_limit=0.0625, scale_limit=0.10, rotate_limit=45
                    ),
                ),
                prob=prob
            ),
